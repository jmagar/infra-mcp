...
async def get_proxy_config_resource(uri: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not device and not path:
            raise ValueError(
                "Invalid SWAG URI format. Use: swag://service_name or swag://device/directory"
            )
        ...
        if device and not path:
            ...
            if service_name == "subdomain-template":
                return await _get_template_resource(
                    swag_device, "_template.subdomain.conf.sample", format_type, include_parsed
                )
            elif service_name == "subfolder-template":
                return await _get_template_resource(
                    swag_device, "_template.subfolder.conf.sample", format_type, include_parsed
                )

            return await _get_service_config_resource(
                swag_device, service_name, force_refresh, include_parsed, format_type
            )
        ...
        # Pattern 5: swag://samples (directory listing of samples)
            ...
            raise ValueError(
                f"Unknown SWAG URI format: {uri}. Use swag://service_name, swag://configs, swag://summary, or swag://samples/"
            )
...
async def _get_template_resource(
    device: str, template_filename: str, format_type: str = "raw", include_parsed: bool = True
) -> dict[str, Any]:
...
async def _get_sample_resource(
    device: str, sample_filename: str, format_type: str = "raw", include_parsed: bool = True
) -> dict[str, Any]:
            ...
            if "|" in line:
                ...
                if len(parts) >= 4:
                    ...
                    if base_name.startswith("_"):
                        ...
                        service_name = base_name
                        display_name = (
                            base_name.replace("_template.", "").replace(".conf", "").title()
                            + " Template"
                        )
                    elif ".subdomain" in base_name:
                        sample_type = "subdomain"
    ...
    try:
        ...
        if config:
            resource_data["database_info"] = {
                ...
                "sync_last_checked": config.sync_last_checked.isoformat()
...
async def _get_direct_file_resource(
    device: str, file_path: str, format_type: str = "raw", include_parsed: bool = True
) -> dict[str, Any]:
...
async def _get_database_config_resource(
    device: str, service_name: str, force_refresh: bool = False, include_parsed: bool = True
) -> dict[str, Any]:
    ...
    try:
        async with get_async_session() as session:
            ...
            needs_refresh = (
                force_refresh
                or not file_info.get("exists", False)
                or (
                    file_info.get("last_modified")
                    and config.last_modified
                    and file_info["last_modified"] > config.last_modified
                )
            )
...
async def _get_proxy_summary_resource(device: str) -> dict[str, Any]:
    ...
    try:
        ...
        _, config_dir = _get_swag_config()
        directory_resource = await _get_directory_listing_resource(device, config_dir)
