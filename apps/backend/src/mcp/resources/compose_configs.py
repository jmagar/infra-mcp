...
async def get_compose_config_resource(uri: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not device:
            raise ValueError("Invalid Docker URI format. Use: docker://device/service or docker://configs")
        ...
        # Pattern 3: docker://device/service - Service compose file
            ...
            raise ValueError(f"Unknown Docker URI format: {uri}. Use docker://device/service, docker://device/stacks, or docker://configs")
        ...
        if include_parsed:
            try:
                ...
                if service_name in services:
                    ...
                    similar_services = [s for s in services.keys() if service_name.lower() in s.lower() or s.lower() in service_name.lower()]
...
async def _resolve_compose_file_path(device: str, service_name: str) -> Optional[str]:
    ...
    logger.info(f"Resolving compose path for {service_name} on {device} - checking running containers")
            ...
            for line in result.stdout.strip().split('\n'):
                ...
                try:
                    ...
                    if service_name.lower() in container_name.lower():
                        ...
                        inspect_result = await execute_ssh_command_simple(device, inspect_cmd, timeout=15)
                        ...
                        if inspect_result.return_code == 0:
                            ...
                            if inspect_data:
                                ...
                                if config_files:
                                    ...
                                    if file_paths:
                                        logger.info(f"Found compose file via container labels: {file_paths[0]}")
                                        return file_paths[0]
...
async def _search_compose_file_in_path(device: str, service_name: str, base_path: str) -> Optional[str]:
        ...
        if result.return_code == 0 and result.stdout.strip() != 'NOT_FOUND':
            ...
            if len(parts) >= 4:
                return {
                    ...
                    'last_modified': datetime.fromtimestamp(int(parts[2]), tz=timezone.utc).isoformat(),
                    'permissions': parts[3],
...
async def _get_device_compose_stacks(device: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            'discovered_stacks': len([s for s in all_stacks.values() if s['status'] == 'discovered']),
        ...
        if result.return_code == 0:
            ...
            for line in result.stdout.strip().split('\n'):
                ...
                try:
                    ...
                    inspect_cmd = f"docker inspect {container_name}"
                    inspect_result = await execute_ssh_command_simple(device, inspect_cmd, timeout=10)
        ...
        async with get_async_session() as session:
            ...
            global_listing.update({
                'summary': {
                    ...
                    'devices_with_compose': len([d for d in global_listing['devices'] if d.get('total_stacks', 0) > 0])
                }
            })
