    ...
    try:
        async with get_async_session() as session:
            ...
            if include_content and file_info.get('exists', False):
                real_time_content = await _get_real_time_file_content(config.device_id, config.file_path)
            ...
            return {
                ...
                'stored_content': config.raw_content if include_content and real_time_content != config.raw_content else None,
                'description': config.description,
                'tags': config.tags,
                'created_at': config.created_at,
                'updated_at': config.updated_at,
...
async def sync_proxy_config(
    ...
    try:
        ...
        async with get_async_session() as session:
            ...
            sync_result['sync_duration_ms'] = int((datetime.now(timezone.utc) - sync_start).total_seconds() * 1000)
            return sync_result
        ...
        async with get_async_session() as session:
            ...
            if not device:
                device_query = select(
                    ...
                    func.count()
                ).group_by(ProxyConfig.device_id)
            ...
            service_query = select(
                ...
                func.count()
            ).group_by(ProxyConfig.service_name)
            if device:
...
async def _sync_configs_to_database(device: str, configs_found: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        async with get_async_session() as session:
            for config_info in configs_found:
                try:
                    ...
                    error_msg = f"Error syncing {config_info.get('service_name', 'unknown')}: {str(e)}"
