...
async def _collect_smart_data(
    ...
    try:
        ...
        smart_result = await ssh_client.execute_command(connection_info, smart_cmd, timeout=timeout)

        # Handle different response scenarios
        if smart_result.return_code != 0:
            if settings.monitoring.smart_graceful_fallback:
                logger.warning(
                    f"SMART command failed for {drive_name} (exit code {smart_result.return_code}), continuing without SMART data"
                )
                return {}
            else:
                raise SystemMonitoringError(
                    f"SMART command failed for {drive_name}: {smart_result.stderr}"
                )
        ...
        if "SMART_ACCESS_DENIED" in smart_output:
            if settings.monitoring.smart_graceful_fallback:
                logger.warning(
                    f"SMART access denied for {drive_name}, continuing without SMART data"
                )
                return {}
            else:
                raise SystemMonitoringError(
                    f"SMART access denied for {drive_name}. Check sudo configuration."
                )

        # Check for permission denied in stderr
        if (
            "Permission denied" in smart_result.stderr
            or "Operation not permitted" in smart_result.stderr
        ):
            if settings.monitoring.smart_graceful_fallback:
                logger.warning(
                    f"SMART permission denied for {drive_name}, continuing without SMART data"
                )
                return {}
            else:
                raise SystemMonitoringError(
                    f"SMART permission denied for {drive_name}. Check sudo configuration."
                )
        ...
        if smart_output and "SMART" in smart_output:
            ...
            logger.debug(
                f"Successfully collected SMART data for {drive_name}: {list(smart_data.keys())}"
            )
        else:
        ...
        if settings.monitoring.smart_graceful_fallback:
            logger.warning(
                f"SMART data collection failed for {drive_name}: {e}, continuing without SMART data"
            )
            return {}
        else:
            logger.error(f"SMART data collection failed for {drive_name}: {e}")
            raise SystemMonitoringError(
                f"SMART data collection failed for {drive_name}: {str(e)}"
            ) from e
                ...
                # For NVMe or other formats: "Temperature: 45 Celsius"
                    ...
                    for i, part in enumerate(parts):
                        if part.isdigit():
                            ...
                            if (
                                i + 1 < len(parts)
                                and parts[i + 1].lower() in ["celsius", "°c", "c"]
                                and 0 <= temp_value <= 100
                            ):
                                smart_data["temperature_celsius"] = temp_value
                                break
...
async def _analyze_connectivity(
    ...
    try:
        ...
        connectivity_results["ping"] = {"status": "error", "error": str(e)}
    ...
    try:
        ssh_test_result = await execute_ssh_command_simple(
            device, "echo 'SSH_CONNECTION_TEST' && whoami && hostname", timeout=15
        )
        ...
        connectivity_results["ssh"] = {"status": "error", "error": str(e)}
...
async def _analyze_system_metrics(ssh_client: SSHClient, connection_info: SSHConnectionInfo) -> dict[str, Any]:
    ...
    try:
        ...
        cpu_result = await ssh_client.execute_command(
            connection_info, "cat /proc/stat | head -1", timeout=10
        )
        ...
        load_result = await ssh_client.execute_command(
            connection_info, "cat /proc/loadavg", timeout=10
        )
    ...
    try:
        mem_result = await ssh_client.execute_command(
            connection_info, "cat /proc/meminfo", timeout=10
        )
    ...
    try:
        ...
        uptime_result = await ssh_client.execute_command(
            connection_info, "cat /proc/uptime", timeout=10
        )
        ...
        boot_time_result = await ssh_client.execute_command(
            connection_info, "stat -c %Y /proc/1", timeout=10
        )
...
async def _analyze_docker(
    ...
    try:
        ...
        if (
            docker_version_result.return_code == 0
            and "DOCKER_NOT_AVAILABLE" not in docker_version_result.stdout
        ):
            ...
            try:
                ...
                if json_start != -1:
                    ...
                    docker_info_data = json.loads(json_data)
            ...
            if compose_result.return_code == 0:
                compose_files = [
                    f.strip() for f in compose_result.stdout.strip().split("\n") if f.strip()
                ]
                compose_base_paths = list({"/".join(f.split("/")[:-1]) for f in compose_files})

                docker_info["compose_projects"] = compose_files
                docker_info["compose_base_paths"] = compose_base_paths
            ...
            docker_info["reason"] = "Docker not available or not running"

    except Exception as e:
        docker_info["error"] = str(e)
        logger.warning(f"Docker analysis failed for {device}: {e}")

    return docker_info, services
...
async def _analyze_storage_info(
    ...
    try:
        ...
        if zfs_result.return_code == 0 and "ZFS_NOT_AVAILABLE" not in zfs_result.stdout:
            ...
            storage_info["zfs_snapshots"] = snapshots[:10]  # Limit to 10 most recent
...
async def _analyze_hardware(
    ...
    try:
        ...
        if hw_result.return_code == 0:
                ...
                if "Model name:" in line:
                    ...
                    if len(mem_parts) >= 3:
                        ...
                        memory_info["available"] = (
                            mem_parts[6] if len(mem_parts) > 6 else mem_parts[3]
                        )
            ...
            hardware_info["gpu_detected"] = len(gpu_info) > 0
...
async def _analyze_virtualization(
    ...
    try:
        ...
        if virt_result.return_code == 0 and "VIRSH_NOT_AVAILABLE" not in virt_result.stdout:
            virt_lines = [
                line.strip() for line in virt_result.stdout.strip().split("\n") if line.strip()
            ]

            virtualization["virsh_available"] = True
            virtualization["vm_list"] = virt_lines[1:]  # Skip header
        else:
            virtualization["virsh_available"] = False
...
async def _analyze_drives(ssh_client: SSHClient, connection_info: SSHConnectionInfo) -> dict[str, Any]:
    ...
    try:
        ...
        smartctl_result = await ssh_client.execute_command(
            connection_info, "which smartctl", timeout=10
        )
        ...
        lsblk_result = await ssh_client.execute_command(
            connection_info, "lsblk -d -n -o NAME,TYPE | grep disk", timeout=15
        )
        drives_to_check = []
        ...
        for drive_path in drives_to_check:
            ...
            try:
                ...
                if smart_available:
                    try:
                        smart_data = await _collect_smart_data(
                            ssh_client, connection_info, drive_path, drive_path
                        )
                        if smart_data:
        ...
        summary = {
            ...
            if failed_drives == 0 and healthy_drives > 0
            else ("failed" if failed_drives > 0 else "unknown"),
        ...
        drive_health["error"] = str(e)
        logger.warning(f"Drive health analysis failed: {e}")
...
async def get_device_info(
    ...
    try:
        ...
        # 1. Network and SSH Connectivity Analysis
        ...
        try:
            system_metrics = await _analyze_system_metrics(ssh_client, connection_info)
        ...
        docker_info, services = await _analyze_docker(device, ssh_client, connection_info)
        results["docker_info"] = docker_info
        results["services"] = services

        # 4. Storage and ZFS Analysis
        logger.info(f"Analyzing storage configuration on {device}")
        storage_info = await _analyze_storage_info(device, ssh_client, connection_info)
        results["storage_info"] = storage_info

        # 5. Hardware Information Analysis
        logger.info(f"Analyzing hardware information on {device}")
        hardware_info = await _analyze_hardware(device, ssh_client, connection_info)
        results["hardware_info"] = hardware_info

        # 6. Operating System Information
        logger.info(f"Analyzing OS information on {device}")
        os_info = await _analyze_os_info(device, ssh_client, connection_info)
        results["os_info"] = os_info

        # 7. Virtualization Detection
        logger.info(f"Analyzing virtualization on {device}")
        virtualization = await _analyze_virtualization(device, ssh_client, connection_info)
        results["virtualization"] = virtualization

        # 8. Drive Health Analysis
        logger.info(f"Analyzing drive health on {device}")
        drive_health = await _analyze_drives(ssh_client, connection_info)
        results["drive_health"] = drive_health

        # 9. Collect top processes if requested
        if include_processes:
            logger.info(f"Collecting process information from {device}")
            processes = await _collect_processes(ssh_client, connection_info)
        ...
        results["analysis_summary"] = {
            ...
            "system_metrics_collected": "system_metrics" in results
            and "error" not in results["system_metrics"],
            "drive_health_checked": "drive_health" in results
            and "error" not in results["drive_health"],
            "processes_included": include_processes,
            "analysis_timestamp": analysis_end.isoformat(),
        }

        # 11. Store results in database if requested
        if store_results:
            try:
        ...
        logger.info(
            f"Comprehensive device info collection completed for {device} in {analysis_duration:.2f}s"
        )
        return results
