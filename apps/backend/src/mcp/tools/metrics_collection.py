...
async def get_system_info(
    ...
    try:
        ...
        for metric_name, command in commands.items():
            try:
                ...
                if result.success:
                    ...
                    logger.warning(f"Failed to collect {metric_name} metrics from {device}: {result.stderr}")
        ...
        parsed_metrics["load_average"] = uptime_info.get("load_average", {"1min": 0.0, "5min": 0.0, "15min": 0.0})
...
async def get_drive_health(
    ...
    try:
        ...
        if drive:
            ...
            try:
                ...
                if discover_result.success and discover_result.stdout:
                    ...
                    logger.warning(f"Could not discover drives on {device}: {discover_result.stderr}")
        ...
        healthy_drives = sum(1 for d in drive_health_data 
...
async def _collect_single_drive_health(
    ...
    try:
        ...
        if not smart_result.success:
            logger.debug(f"SMART not available for drive {drive_name} on {device}: {smart_result.stderr}")
            return {
                "device": device,
                "drive": drive_name,
                "model": "unknown",
        ...
        if attributes_result.success:
            smart_attributes = _parse_smart_attributes(
                attributes_result.stdout, 
                drive_name.startswith('nvme')
            )
            drive_info.update(smart_attributes)
        else:
            logger.debug(f"Could not get SMART attributes for {drive_name} on {device}: {attributes_result.stderr}")
...
def _parse_journalctl_json_entry(log_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        service = (
            log_data.get('_SYSTEMD_UNIT', '') or
            log_data.get('SYSLOG_IDENTIFIER', '') or
            log_data.get('_COMM', '') or
            'unknown'
        )
