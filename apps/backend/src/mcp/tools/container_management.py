...
from apps.backend.src.core.exceptions import (
    ContainerError, DeviceNotFoundError, SSHConnectionError
)
...
async def list_containers(
    ...
    try:
        ...
        # Apply pagination
        paginated_containers = containers[offset:offset + limit] if limit is not None else containers[offset:]
...
async def get_container_info(
    ...
    try:
        ...
        if not inspection_data or not isinstance(inspection_data, list) or len(inspection_data) == 0:
            raise ContainerError(
        ...
        logger.error(f"Unexpected error getting container details for '{container_name}' on {device}: {e}")
        raise ContainerError(
...
async def get_container_logs(
    ...
    try:
        ...
        logger.error(f"Unexpected error getting logs for container '{container_name}' on {device}: {e}")
        raise ContainerError(
...
async def get_service_dependencies(
    ...
    Raises:
        ...
        SSHConnectionError: If SSH connection fails
    """
    logger.info(f"Analyzing service dependencies for container '{container_name}' on device: {device}")
    ...
    try:
        ...
        for container_info in containers_data:
            ...
            try:
                ...
                if not detail_result.success:
                    logger.warning(f"Failed to inspect container {container_id}: {detail_result.error_message}")
                    continue
                ...
                if service_name and project_name:
                    ...
                    service_containers[service_key] = {
                        ...
                        "volumes": [mount.get("Source", "") for mount in mounts if mount.get("Type") == "bind" or mount.get("Type") == "volume"],
        ...
        if not target_service_key or target_service_key not in service_containers:
            ...
            target_service_info = {
                ...
                "volumes": [mount.get("Source", "") for mount in target_mounts if mount.get("Type") in ["bind", "volume"]],
            ...
            target_volumes = set(mount.get("Source", "") for mount in target_mounts if mount.get("Type") in ["bind", "volume"])
            volume_deps = []
            ...
            response = {
                ...
                "query_info": {
                    ...
                    "timestamp": datetime.now(timezone.utc).isoformat(),
                    "execution_time_ms": int((containers_result.raw_result.execution_time + target_details_result.raw_result.execution_time) * 1000)
                }
            }
            
            logger.info(f"Analyzed standalone container '{container_name}' on {device} (found {len(network_deps)} network peers, {len(volume_deps)} volume peers)")
            return response
        ...
        project_services = [s for s in service_containers.values() if s["project_name"] == target_project_name]
            ...
            for network_name in service_networks:
                if network_name in network_mappings:
                    for related_service_key in network_mappings[network_name]:
                        if (related_service_key != service_key and 
                            related_service_key in service_containers and
                            service_containers[related_service_key]["project_name"] == target_project_name):
                            
                            dependency_graph["relationships"].append({
                                "from_service": service["service_name"],
                                "to_service": service_containers[related_service_key]["service_name"],
                                "type": "network",
        ...
        logger.error(f"Unexpected error analyzing service dependencies for '{container_name}' on {device}: {e}")
        raise ContainerError(
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "status": {
                    ...
                    "enum": ["running", "exited", "paused", "restarting", "removing", "dead", "created"]
                },
                "all_containers": {
                    "type": "boolean",
