...
class ProxyConfig(Base):
    ...
    changes = relationship("ProxyConfigChange", back_populates="config", cascade="all, delete-orphan")
    ...
    def validate_names(self, key, value):
        ...
        if not value.replace('-', '').replace('_', '').isalnum():
            raise ValueError(f"{key} must contain only alphanumeric characters, hyphens, and underscores")
...
class ProxyConfigChange(Base):
    ...
    # Primary fields
    id = Column(Integer, primary_key=True, autoincrement=True)
    config_id = Column(Integer, ForeignKey("proxy_configs.id", ondelete="CASCADE"), nullable=False, index=True)
    
    # Change information
    change_type = Column(String(50), nullable=False, index=True)  # created, modified, deleted, synced
    ...
    __table_args__ = (
        CheckConstraint("change_type IN ('created', 'modified', 'deleted', 'synced', 'error')", name='ck_proxy_change_type'),
...
class ProxyConfigTemplate(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("config_type IN ('subdomain', 'subfolder', 'port', 'custom')", name='ck_template_config_type'),
...
def add_proxy_config_relationship():
    ...
    try:
        ...
        if not hasattr(Device, 'proxy_configs'):
            Device.proxy_configs = relationship("ProxyConfig", back_populates="device", cascade="all, delete-orphan")
    except ImportError:
