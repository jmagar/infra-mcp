...
from sqlalchemy import (
    Column, String, DateTime, Integer, BigInteger, Boolean, Text, JSON, 
...
class DataCollectionAudit(Base):
    ...
    def performance_score(self) -> float:
        ...
        duration_score = max(0, 1000 - self.duration_ms) / 1000
        
        # Bonus for cache hits
        cache_bonus = 0.5 if self.cache_hit else 0.0
...
class ServicePerformanceMetric(Base):
    ...
    def cache_hit_rate(self) -> float:
        ...
        if self.operations_total == 0:
            return 0.0
        return self.operations_cached / self.operations_total
    
    @property
    def ssh_connection_reuse_rate(self) -> float:
        """Calculate SSH connection reuse rate"""
        total_connections = self.ssh_connections_created + self.ssh_connections_reused
        if total_connections == 0:
            return 0.0
        return self.ssh_connections_reused / total_connections
...
class CacheMetadata(Base):
    ...
    def update_access_stats(self) -> None:
        ...
        if self.access_count > 0:
            ...
            self.avg_access_interval = int(total_time / self.access_count)
        ...
        if self.access_count > 1:
            self.hit_rate = min(100.0, (self.access_count - 1) / self.access_count * 100)
