...
class NginxConfigParser:
    ...
    def _parse_location_blocks(self, content: str) -> list[dict[str, Any]]:
        ...
        location_pattern = re.compile(
            r"location\s+([^{]+)\s*\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}", re.MULTILINE | re.DOTALL
        )
    ...
    def _parse_upstream_blocks(self, content: str) -> list[dict[str, Any]]:
        ...
        upstream_pattern = re.compile(
            r"upstream\s+([^{]+)\s*\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}", re.MULTILINE | re.DOTALL
        )
    ...
    def _extract_service_info_from_path(self, file_path: str) -> dict[str, str]:
        ...
        return {"service_name": service_name, "subdomain": subdomain, "config_filename": path.name}
        ...
        try:
            ...
            if brace_count != 0:
                ...
                validation["errors"].append(
                    f"Unbalanced braces: {abs(brace_count)} {'opening' if brace_count > 0 else 'closing'} braces"
                )
                ...
                if line and not line.startswith("#") and not line.endswith((";", "{", "}")):
                    ...
                    if not any(block in line for block in ["server", "location", "upstream", "if"]):
                        validation["warnings"].append(
                            f"Line {line_num}: Possible missing semicolon"
                        )
        ...
        for match in matches:
            ...
            if match.startswith("http://") or match.startswith("https://"):
                ...
                if url_match:
                    ...
                    dependencies.append(
                        {"type": "direct", "host": host, "port": port, "full_url": match}
                    )
            elif match.startswith("$"):
                # Variable reference
                dependencies.append({"type": "variable", "reference": match, "full_url": match})
            else:
                # Upstream reference
                dependencies.append({"type": "upstream", "upstream_name": match, "full_url": match})
