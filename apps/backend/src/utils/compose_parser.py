        ...
        try:
            ...
            if not isinstance(parsed_yaml, dict):
                raise ComposeParseError(
                    "Docker Compose file must contain a YAML object at root level"
                )
        ...
        for port in ports:
            if isinstance(port, str):
                ...
                if ":" in port:
                    ...
                    extracted.append(
                        {"host_port": port, "container_port": port, "protocol": "tcp", "raw": port}
                    )
                ...
                extracted.append(
                    {
                        ...
                        "mode": volume.get("bind", {}).get("propagation")
    ...
    def _validate_compose_structure(self, parsed_yaml: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not version:
            validation["warnings"].append(
                "No version specified - Docker Compose will use legacy format"
            )
        elif version.startswith("1."):
            validation["warnings"].append(
                f"Docker Compose version {version} is legacy - consider upgrading"
            )
        ...
        for service_name, service_config in services.items():
            if not isinstance(service_config, dict):
                validation["errors"].append(
                    f"Service '{service_name}' configuration must be an object"
                )
                validation["valid"] = False
                continue
            ...
            for port in ports:
                if isinstance(port, str) and ":" in port:
                    ...
                    if host_port in host_ports:
                        validation["warnings"].append(
                            f"Service '{service_name}' has duplicate host port: {host_port}"
                        )
