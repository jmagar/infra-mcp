...
class DeviceManager:
    ...
    async def register_device(
        ...
        async with get_async_session() as session:
            ...
            if test_connectivity:
                ...
                device.last_seen = datetime.now(timezone.utc) if connectivity_result["connected"] else None
    ...
    async def get_device_statistics(self) -> Dict[str, Any]:
        ...
        async with get_async_session() as session:
            ...
            status_counts = {}
            result = await session.execute(
                select(Device.status, sa_func.count(Device.id))
            ...
            type_counts = {}
            result = await session.execute(
                select(Device.device_type, sa_func.count(Device.id))
