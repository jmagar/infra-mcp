...
class SSHConnectionPool:
                            ...
                            async with self._pool_locks.get(host, asyncio.Lock()):
                                if connections:
                                    ...
                                    try:
                                        ...
                                        logger.debug(f"Error closing idle connection to {host}: {e}")
                                    
                                    self._connection_counts[host] = max(0, self._connection_counts.get(host, 0) - 1)
    ...
    @asynccontextmanager
    async def get_connection(self, connection_info: SSHConnectionInfo) -> AsyncGenerator[SSHClientConnection, None]:
        """
        ...
        async with self._execution_semaphore:
            ...
            for attempt in range(retries + 1):
                ...
                try:
                    ...
                    execution_time = time.time() - start_time
                    last_exception = e
                    error_msg = f"Command timeout after {timeout}s on {connection_info.host}: {command}"
        ...
        for command in commands:
            try:
                ...
                if fail_fast:
                    ...
                    results.append(SSHExecutionResult(
                        command=command,
                        return_code=-1,
                        stdout="",
                        stderr=str(e),
    ...
    async def execute_parallel(
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                processed_results.append(SSHExecutionResult(
                    command=command,
                    return_code=-1,
                    stdout="",
                    stderr=str(result),
...
async def execute_ssh_command_simple(hostname: str, command: str, timeout: int = 120) -> SSHExecutionResult:
    """
