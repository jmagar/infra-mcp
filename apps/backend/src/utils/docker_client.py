...
from apps.backend.src.utils.ssh_client import (
    SSHClient, SSHConnectionInfo, SSHExecutionResult, get_ssh_client
)
from apps.backend.src.core.exceptions import (
    ContainerError, SSHConnectionError, SSHCommandError, DeviceNotFoundError
)
...
class DockerClient:
    ...
    def __init__(self, ssh_client: Optional[SSHClient] = None):
        ...
        self._error_patterns = {
            ...
            "invalid_container_id": [
                r"invalid container id",
                r"Error parsing reference"
            ],
            "network_error": [
                r"network (.+) not found",
                r"No such network: (.+)"
    ...
    def _parse_json_output(self, output: str, command_type: DockerCommandType) -> Optional[Union[Dict, List]]:
        """
        ...
        try:
            ...
            if command_type == DockerCommandType.LIST:
                ...
                container_blocks = [block.strip() for block in output.strip().split('---') if block.strip()]
        ...
        **kwargs
        ...
        try:
            ...
            if parse_json and ssh_result.stdout:
                ...
                docker_result.parsed_data = self._parse_json_output(
                    ssh_result.stdout, command_type
                )
                ...
                if docker_result.parsed_data is None and ssh_result.stdout.strip():
                    ...
                    if isinstance(docker_result.parsed_data, list):
                        logger.debug(f"Successfully parsed {len(docker_result.parsed_data)} JSON items")
                    else:
