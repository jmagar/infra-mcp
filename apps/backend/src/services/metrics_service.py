...
from apps.backend.src.schemas.system_metrics import (
    SystemMetricResponse, SystemMetricsList
)
from apps.backend.src.schemas.drive_health import (  
    DriveHealthResponse, DriveHealthList, DriveInventory
)
    ...
    async def get_device_metrics(
        ...
        if live:
            ...
            try:
                ...
                memory_usage = float(memory_result.stdout.strip()) if memory_result.stdout.strip() else 0.0
    ...
    async def get_device_drives(
        ...
        if live:
            ...
            try:
                ...
                drives_data = json.loads(drives_result.stdout) if drives_result.stdout else {"blockdevices": []}
                smart_devices = smart_result.stdout.strip().split('\n') if smart_result.stdout.strip() else []
    ...
    async def get_device_network(
        ...
        try:
            ...
            for iface in interfaces_data:
                interfaces.append(NetworkInterfaceResponse(
                    device_id=device_id,
            ...
            if result.stdout:
                ...
                for line in lines:
                    if line.strip().startswith('pool:'):
                        ...
                        pools.append(ZFSStatusResponse(
                            device_id=device_id,
                            **current_pool,
                            last_scrub=None,
                            next_scrub=None,
                    ...
                    if line.strip():
                        ...
                        if len(parts) >= 3:
                            snapshots.append({
                                ...
                                "snapshot": parts[0].split('@')[1] if '@' in parts[0] else parts[0]
                            })
                ...
                for line in lines:
                    if line.strip():
                        logs.append(SystemLogResponse(
                            device_id=device_id,
                            timestamp=datetime.now(timezone.utc),  # Would parse actual timestamp
