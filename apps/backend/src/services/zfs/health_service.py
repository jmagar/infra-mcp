...
class ZFSHealthService(ZFSBaseService):
    ...
    async def check_zfs_health(self, hostname: str, timeout: int = 60) -> Dict[str, Any]:
        ...
        try:
            ...
            pools_list = await self._execute_zfs_command(hostname, "zpool list -H -o name,health", timeout)
            ...
            raise ZFSError(f"Failed to check ZFS health: {str(e)}", operation="check_health", hostname=hostname)
    ...
    async def get_arc_stats(self, hostname: str, timeout: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ZFSError(f"Failed to get ARC stats: {str(e)}", operation="get_arc_stats", hostname=hostname)
    ...
    async def monitor_zfs_events(self, hostname: str, timeout: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ZFSError(f"Failed to monitor ZFS events: {str(e)}", operation="monitor_events", hostname=hostname)
    ...
    async def scrub_pool(self, hostname: str, pool_name: str, timeout: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ZFSError(f"Failed to start scrub: {str(e)}", operation="scrub_pool", hostname=hostname)

    async def get_scrub_status(self, hostname: str, pool_name: str, timeout: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ZFSError(f"Failed to get scrub status: {str(e)}", operation="get_scrub_status", hostname=hostname)