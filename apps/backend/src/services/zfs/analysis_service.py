...
class ZFSAnalysisService(ZFSBaseService):
    ...
    async def generate_zfs_report(self, hostname: str, timeout: int = 120) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ZFSError(f"Failed to generate ZFS report: {str(e)}", operation="generate_report", hostname=hostname)
    ...
    async def analyze_snapshot_usage(self, hostname: str, timeout: int = 60) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                sorted_snapshots = sorted(snapshots, key=lambda x: self._parse_size(x["used"]), reverse=True)
            ...
            raise ZFSError(f"Failed to analyze snapshot usage: {str(e)}", operation="analyze_snapshot_usage", hostname=hostname)
    ...
    async def optimize_zfs_settings(self, hostname: str, timeout: int = 60) -> Dict[str, Any]:
        ...
        try:
            ...
            for pool in pools:
                capacity = float(pool["capacity"]) if pool["capacity"].replace('.', '').isdigit() else 0
                if capacity > 80:
                    recommendations.append({
                        "type": "capacity",
            ...
            raise ZFSError(f"Failed to optimize ZFS settings: {str(e)}", operation="optimize_settings", hostname=hostname)

    async def get_dataset_usage_trends(self, hostname: str, dataset_name: str, timeout: int = 60) -> Dict[str, Any]:
        """Analyze dataset usage trends and growth patterns"""
        try:
            # Get current dataset properties
            dataset_props = await self.dataset_service.get_dataset_properties(hostname, dataset_name, timeout)
            ...
            self.logger.error(f"Error analyzing dataset usage trends for {dataset_name} on {hostname}: {e}")
            raise ZFSError(f"Failed to analyze dataset usage trends: {str(e)}", operation="get_dataset_usage_trends", hostname=hostname)