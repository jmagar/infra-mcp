...
from apps.backend.src.core.exceptions import (
    SSHConnectionError, SSHCommandError, ZFSError
)
    ...
    async def _execute_zfs_command(self, hostname: str, command: str, timeout: int = 30) -> str:
        ...
        try:
            ...
            if result.return_code != 0:
                ...
                raise ZFSError(f"ZFS command failed: {error_msg}", operation="zfs_command", hostname=hostname)
            ...
            raise ZFSError(f"ZFS command error on {hostname}: {str(e)}", operation="zfs_command", hostname=hostname)
