...
class ZFSPoolService(ZFSBaseService):
    ...
    async def list_pools(self, hostname: str, timeout: int = 30) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise ZFSError(f"Failed to list ZFS pools: {str(e)}", operation="list_pools", hostname=hostname)

    async def get_pool_status(self, hostname: str, pool_name: str, timeout: int = 30) -> Dict[str, Any]:
        """Get detailed status for a specific ZFS pool"""
        try:
            # Get detailed pool status
            status_output = await self._execute_zfs_command(
                hostname,
                f"zpool status -v {pool_name}",
                timeout
            )
            ...
            props_output = await self._execute_zfs_command(
                hostname,
                f"zpool get all {pool_name} -H -o property,value",
                timeout
            )
            ...
            raise ZFSError(f"Failed to get pool status: {str(e)}", operation="get_pool_status", hostname=hostname)

    async def get_pool_properties(self, hostname: str, pool_name: str, timeout: int = 30) -> Dict[str, Any]:
        """Get all properties for a specific pool"""
        try:
            output = await self._execute_zfs_command(
                hostname,
                f"zpool get all {pool_name} -H -o property,value,source",
                timeout
            )
            ...
            raise ZFSError(f"Failed to get pool properties: {str(e)}", operation="get_pool_properties", hostname=hostname)