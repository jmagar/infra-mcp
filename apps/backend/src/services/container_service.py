...
class ContainerService:
    ...
    async def list_containers(
        ...
        query = (
            query.order_by(desc(ContainerSnapshot.time))
        ...
        container_responses = [
            ContainerSnapshotResponse.model_validate(container) for container in containers
        ]
                            ...
                            try:
                                ...
                                if stats_result.stdout:
                                    try:
                                        ...
                                        logger.warning(
                                            f"Failed to parse stats for container {container_data.get('ID')}"
                                        )
                                ...
                                containers.append(
                                    ContainerSummary(
                                        ...
                                        memory_usage_percent=(
                                            memory_usage_mb / memory_limit_mb * 100
                                        )
                                        ...
                                        exposed_ports=container_data.get("Ports", "").split(", ")
            ...
            for container in containers:
                summaries.append(
                    ContainerSummary(
                        ...
                        memory_usage_mb=container.memory_usage_bytes / (1024 * 1024)
                        if container.memory_usage_bytes
                        else 0.0,
                        memory_limit_mb=container.memory_limit_bytes / (1024 * 1024)
                        if container.memory_limit_bytes
                        else 0.0,
                        memory_usage_percent=(
                            container.memory_usage_bytes / container.memory_limit_bytes * 100
                        )
                        if container.memory_limit_bytes and container.memory_usage_bytes
                        else 0.0,
                        network_io_mb={
                            "rx": container.network_bytes_recv / (1024 * 1024)
                            if container.network_bytes_recv
                            else 0.0,
                            "tx": container.network_bytes_sent / (1024 * 1024)
                            if container.network_bytes_sent
                            else 0.0,
                        },
                        block_io_mb={
                            "read": container.block_read_bytes / (1024 * 1024)
                            if container.block_read_bytes
                            else 0.0,
                            "write": container.block_write_bytes / (1024 * 1024)
                            if container.block_write_bytes
                            else 0.0,
                        },
                        exposed_ports=[
                            f"{port['HostPort']}:{port['PrivatePort']}"
    ...
    async def get_container_info(
        self, device_id: UUID, container_name: str, live_data: bool = False
    ) -> ContainerDetails:
        ...
        if live_data:
            ...
            try:
                ...
                try:
                    ...
                    raise ContainerError(
                        f"Failed to parse container details for '{container_name}'"
                    )
                ...
                return ContainerDetails(
                    ...
                    created_at=datetime.fromisoformat(
                        container_data.get("Created", "").replace("Z", "+00:00")
                    )
                    ...
                    if state.get("StartedAt") and state.get("StartedAt") != "0001-01-01T00:00:00Z"
                    else None,
                    ...
                    cpu_limit=host_config.get("CpuQuota", 0) / 100000.0
                    if host_config.get("CpuQuota") and host_config.get("CpuQuota") > 0
                    else None,
                ...
                logger.error(
                    f"Error getting container details for {container_name} on device {device_id}: {e}"
                )
            ...
            query = (
                ...
                .order_by(desc(ContainerSnapshot.time))
                .limit(1)
            )
    ...
    async def get_container_logs(
        ...
        try:
            ...
            logger.error(
                f"Error getting container logs for {container_name} on device {device_id}: {e}"
            )
