    ...
    try:
        ...
        components = {
            ...
            "device_connectivity": {
                ...
                "score": min(10, (device_stats.online_devices / max(device_stats.total_devices, 1)) * 10),
                "details": {
        ...
        async with session_factory() as db:
            ...
            for device in monitored_devices:
                ...
                device_health.append({
                    ...
                    "health_score": 10 if recent_metrics > 0 and device.status == "online" else 0
                })
...
async def performance_metrics(
    ...
    try:
        ...
        return {
            ...
            "recommendations": [
                "Database queries performing well" if db_query_time < 50 else "Consider database optimization",
                "SSH cache working efficiently" if cache_efficiency > 50 else "Consider increasing cache TTL",
                "System performance nominal" if total_response_time < 200 else "Monitor API response times"
            ]
        }
    ...
    try:
        ...
        dashboard_data = {
            "overview": {
                ...
                "connectivity_percentage": round((device_summary.online / max(device_summary.total, 1)) * 100, 1)
            },
            "data_collection": {
            ...
            "quick_actions": [
                "All systems operational" if all(status == "healthy" for status in health_indicators.values())
            ...
            "alerts": [
                ...
                if device_summary.total - device_summary.online > 0 else None,
        ...
        # Remove None alerts
        dashboard_data["alerts"] = [alert for alert in dashboard_data["alerts"] if alert is not None]
