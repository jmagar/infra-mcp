...
class ContainerSnapshotBase(BaseModel):
    ...
    def validate_status(cls, v):
        if v is not None:
            valid_statuses = [
                "running", "paused", "restarting", "removing", "dead", 
                "created", "exited", "stopped"
            ]
            if v.lower() not in valid_statuses:
                raise ValueError(f"Invalid container status. Valid statuses: {', '.join(valid_statuses)}")
            return v.lower()
        return v
    ...
    def validate_state(cls, v):
        if v is not None:
            ...
            if v.lower() not in valid_states:
                raise ValueError(f"Invalid container state. Valid states: {', '.join(valid_states)}")
            return v.lower()
        return v
