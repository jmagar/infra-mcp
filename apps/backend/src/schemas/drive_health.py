...
from apps.backend.src.schemas.common import HealthStatus, PaginatedResponse, TimeRangeParams
...
class DriveHealthBase(BaseModel):
    ...
    # Drive information
    drive_type: str | None = Field(None, description="Drive type (ssd, hdd, nvme)")
    model: str | None = Field(None, max_length=255, description="Drive model")
    serial_number: str | None = Field(None, max_length=255, description="Drive serial number")
    capacity_bytes: int | None = Field(None, ge=0, description="Drive capacity in bytes")

    # Health metrics
    temperature_celsius: int | None = Field(None, description="Drive temperature in Celsius")
    power_on_hours: int | None = Field(None, ge=0, description="Power-on hours")
    total_lbas_written: int | None = Field(None, ge=0, description="Total LBAs written")
    total_lbas_read: int | None = Field(None, ge=0, description="Total LBAs read")
    reallocated_sectors: int | None = Field(None, ge=0, description="Reallocated sectors count")
    pending_sectors: int | None = Field(None, ge=0, description="Pending sectors count")
    uncorrectable_errors: int | None = Field(
        None, ge=0, description="Uncorrectable errors count"
    )

    # Status indicators
    smart_status: str | None = Field(None, description="SMART status (PASSED/FAILED/UNKNOWN)")
    smart_attributes: dict[str, Any] = Field(default_factory=dict, description="SMART attributes")
    health_status: HealthStatus = Field(
        default=HealthStatus.UNKNOWN, description="Overall health status"
    )

    @field_validator("drive_name")
    @classmethod
    def validate_drive_name(cls, v: str) -> str:
    ...
    @classmethod
    def validate_drive_type(cls, v: str | None) -> str | None:
        if v is not None:
    ...
    @classmethod
    def validate_smart_status(cls, v: str | None) -> str | None:
        if v is not None:
    ...
    @classmethod
    def validate_temperature(cls, v: int | None) -> int | None:
        if v is not None:
...
class DriveHealthCreate(DriveHealthBase):
    ...
    time: datetime = Field(default_factory=lambda: datetime.now(UTC), description="Measurement timestamp")
...
class DriveHealthQuery(TimeRangeParams):
    ...
    drive_names: list[str] | None = Field(None, description="Filter by drive names")
    drive_types: list[str] | None = Field(None, description="Filter by drive types")
    health_status: list[HealthStatus] | None = Field(None, description="Filter by health status")
    smart_status: list[str] | None = Field(None, description="Filter by SMART status")
    min_temperature: int | None = Field(None, description="Minimum temperature filter")
    max_temperature: int | None = Field(None, description="Maximum temperature filter")
...
class DriveHealthSummary(BaseModel):
    ...
    drive_type: str | None = Field(description="Drive type")
    model: str | None = Field(description="Drive model")
    capacity_gb: float | None = Field(description="Drive capacity in GB")

    # Current status
    health_status: HealthStatus = Field(description="Overall health status")
    smart_status: str | None = Field(description="SMART status")
    current_temperature: int | None = Field(description="Current temperature")

    # Wear indicators
    power_on_hours: int | None = Field(description="Total power-on hours")
    reallocated_sectors: int | None = Field(description="Reallocated sectors")
    pending_sectors: int | None = Field(description="Pending sectors")
    uncorrectable_errors: int | None = Field(description="Uncorrectable errors")
...
class DriveHealthTrends(BaseModel):
    ...
    health_status_history: list[dict[str, Any]] = Field(
        description="Health status changes over time"
    )
    smart_status_history: list[dict[str, Any]] = Field(description="SMART status changes over time")

    # Predictive indicators
    estimated_remaining_life: float | None = Field(
        description="Estimated remaining life percentage"
    )
    wear_leveling_indicator: float | None = Field(description="Wear leveling indicator")
...
class DriveHealthAlert(BaseModel):
    ...
    current_value: float | None = Field(description="Current metric value")
    threshold_value: float | None = Field(description="Threshold that was exceeded")
...
class DriveHealthThresholds(BaseModel):
    ...
    # Sector thresholds
    reallocated_sectors_warning: int = Field(
        default=5, description="Reallocated sectors warning threshold"
    )
    reallocated_sectors_critical: int = Field(
        default=20, description="Reallocated sectors critical threshold"
    )
    pending_sectors_warning: int = Field(default=1, description="Pending sectors warning threshold")
    pending_sectors_critical: int = Field(
        default=5, description="Pending sectors critical threshold"
    )

    # Error thresholds
    uncorrectable_errors_warning: int = Field(
        default=0, description="Uncorrectable errors warning threshold"
    )
    uncorrectable_errors_critical: int = Field(
        default=1, description="Uncorrectable errors critical threshold"
    )

    # Wear thresholds (for SSDs)
    wear_leveling_warning: int = Field(
        default=80, description="Wear leveling warning threshold (%)"
    )
    wear_leveling_critical: int = Field(
        default=90, description="Wear leveling critical threshold (%)"
    )
    ...
    def validate_reallocated_critical(cls, v: int, info) -> int:
        if (
            ...
            raise ValueError(
                "Reallocated sectors critical threshold must be higher than warning threshold"
            )
        return v
...
class SmartAttribute(BaseModel):
    ...
    when_failed: str | None = Field(description="When the attribute failed (if applicable)")
