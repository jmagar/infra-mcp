...
class SystemMetricBase(BaseModel):
    ...
    disk_available_bytes: Optional[int] = Field(None, ge=0, description="Available disk space in bytes")
    ...
    # Additional metrics
    additional_metrics: Dict[str, Any] = Field(default_factory=dict, description="Additional metric data")
...
class SystemMetricCreate(SystemMetricBase):
    ...
    time: datetime = Field(default_factory=datetime.utcnow, description="Metric timestamp")
...
class SystemMetricsQuery(TimeRangeParams):
    ...
    def validate_metrics(cls, v):
        if v:
            valid_metrics = [
                "cpu_usage_percent", "memory_usage_percent", "memory_total_bytes", "memory_available_bytes",
                "load_average_1m", "load_average_5m", "load_average_15m",
                "disk_usage_percent", "disk_total_bytes", "disk_available_bytes",
                "network_bytes_sent", "network_bytes_recv", "uptime_seconds", "process_count"
            ]
            for metric in v:
...
class SystemMetricsThresholds(BaseModel):
    ...
    memory_warning: float = Field(default=85.0, ge=0, le=100, description="Memory warning threshold")
    memory_critical: float = Field(default=95.0, ge=0, le=100, description="Memory critical threshold")
