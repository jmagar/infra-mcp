...
class NginxDirective(BaseModel):
    ...
    context: str = Field(..., description="Context where directive appears (server, location, etc.)")
...
class ProxyConfigParsed(BaseModel):
    ...
    upstream_servers: List[str] = Field(default_factory=list, description="Upstream server definitions")
    custom_directives: List[NginxDirective] = Field(default_factory=list, description="Custom nginx directives")
...
class ProxyConfigBase(BaseModel):
    ...
    config_type: ProxyConfigType = Field(default=ProxyConfigType.SUBDOMAIN, description="Configuration type")
    status: ProxyConfigStatus = Field(default=ProxyConfigStatus.ACTIVE, description="Configuration status")
...
class ProxyConfigResponse(ProxyConfigBase):
    ...
    parsed_config: Optional[ProxyConfigParsed] = Field(None, description="Parsed configuration data")
...
class ProxyConfigSummary(BaseModel):
    ...
    by_service_type: Dict[str, int] = Field(default_factory=dict, description="Configurations per service type")
...
class ProxyConfigChange(BaseModel):
    ...
    changes_detected: List[str] = Field(default_factory=list, description="List of detected changes")
...
class ProxyConfigTemplate(BaseModel):
    ...
    example_values: Dict[str, str] = Field(default_factory=dict, description="Example variable values")