 ...
 * Enhanced information-dense dashboard with real-time system insights
 */
    ...
    >
      ...
      <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8">
        ...
        <MetricCard
          ...
          subtitle={`${overview?.totalDevices || 0} total`}
          icon={Server}
          status={(overview?.onlineDevices || 0) === (overview?.totalDevices || 0) ? 'success' : 'warning'}
        ...
        <MetricCard
          ...
          subtitle={`${overview?.totalContainers || 0} total`}
          icon={Box}
          status={(overview?.runningContainers || 0) > 0 ? 'success' : 'warning'}
        ...
        <MetricCard
          ...
          unit="%"
          icon={Cpu}
          status={(overview?.avgCpuUsage || 0) < 70 ? 'success' : (overview?.avgCpuUsage || 0) < 90 ? 'warning' : 'error'}
        ...
        <MetricCard
          ...
          unit="%"
          icon={MemoryStick}
          status={(overview?.avgMemoryUsage || 0) < 80 ? 'success' : 'warning'}
        ...
        <MetricCard
          title="Storage"
          value={overview?.storageUsagePercent || 0}
          unit="%"
          icon={HardDrive}
          status={(overview?.storageUsagePercent || 0) < 80 ? 'success' : 'warning'}
